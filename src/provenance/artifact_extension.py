...
class ArtifactExtension:
    ...
    def from_provenance_formatted_string(data: str) -> provenance.artifact.Artifact:
        ...
        if len(tokens) < 4:
            raise ValueError("Data is not in the expected format. Expected format: [artifact_name]@[artifact_type]@[action_type]@[actor_type]")
        ...
        if not all(tokens):
            raise ValueError("Data contains empty tokens. Expected format: [artifact_name]@[artifact_type]@[action_type]@[actor_type]") 
        try:
            ...
            raise ValueError(f"Error: Data is not in the expected format while parsing artifact: {e}")
